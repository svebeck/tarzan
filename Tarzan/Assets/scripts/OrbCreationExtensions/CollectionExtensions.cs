...
namespace OrbCreationExtensions
{
    public static class CollectionExtensions
    {
        ...
        public static uint GetUInt(this Hashtable hash, object key) {
            return GetUInt(hash, key, 0);
        }
        public static uint GetUInt(this Hashtable hash, object key, uint defaultValue) {
            object value = null;
            if(hash.ContainsKey(key)) {
                value = hash[key];
                if(value == null) return defaultValue;
                if(value.GetType() == typeof(string)) return ((string)value).MakeUInt();
                if(value.GetType() == typeof(bool) && (bool)value) return 0xFFFFFF;
                if(value.GetType() == typeof(uint) || value.GetType() == typeof(long) || value.GetType() == typeof(int)) return (uint)value;
                if(value.GetType() == typeof(float) || value.GetType() == typeof(double)) return Convert.ToUInt32((float)value);
            }
            return defaultValue;
        }
